...
function findShortestPathDFS(input: string[], recordedMoves: Array<string>, currLocation: [number,number]): number {
    var newRecord = JSON.parse(JSON.stringify(recordedMoves));
    var newLocation: [number,number] = [currLocation[0],currLocation[1]];
...
function findShortestPath(input: string[], currLocation: [number,number,number]): number {
    var queue: Array<[number,number,number]> = [];
    queue.push(currLocation);
    var closed = new Set<string>();
...
function anyA(input:string[]): number {
    var smallest = 1000;
