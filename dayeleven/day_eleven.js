...
function calculateMonkeyBusiness(input) {
    var monkeyCounts = [];
    var monkeyHoldings = [];
    var monkeyFunctions = [];
    var monkeyTests = [];
    for (var i = 1; i < input.length; i += 6) {
        var startingItems = input[i].split(/ |,/).filter(function (c) { return c; });
        var currentMonkeyHolding = [];
        for (var j = 2; j < startingItems.length; j++) {
            currentMonkeyHolding.push(+startingItems[j]);
        }
        monkeyHoldings.push(currentMonkeyHolding);
        var currentMonkeyFunction = input[i + 1].split(/ /).filter(function (c) { return c; });
        monkeyFunctions.push(currentMonkeyFunction);
        monkeyCounts.push(0);
        var currMonkeytests = [];
        var currentMonkeyTest = input[i + 2].split(/ /).filter(function (c) { return c; });
        var currentMonkeyTrue = input[i + 3].split(/ /).filter(function (c) { return c; });
        var currentMonkeyFalse = input[i + 4].split(/ /).filter(function (c) { return c; });
