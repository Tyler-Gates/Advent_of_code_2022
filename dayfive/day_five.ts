...
function getStartingPosition(input: string[]): Array<Array<string>> {
...
function doMoves(input: string[], startingPosition: Array<Array<string>>): Array<Array<string>>{
    for (var i = 0; i < input.length; i++) {
        if (input[i][0] == "m") {
            for (var j = i; j < input.length; j++) {
                var current = input[j].replace("move ", "").replace("from ", "").replace("to ", "").split(" ").filter(c => c);
...
function getTopCrates(startingPosition: Array<Array<string>>) {
    var answer = "";
